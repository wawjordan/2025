%% Testing script for the new geometry loading and calculation (05/29/2024)
clc; clear; close all;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
grid_file = fullfile(parent_dir,'anisotropic\constrained_least_squares\reconstruction_surgery\grid_derived_type\kt.grd');
grid_t_file = fullfile(parent_dir,'anisotropic\constrained_least_squares\reconstruction_surgery\GRID.mat');
...
block_info_list(1).connected_bc_list(1)                 = connected_bc_info_t_old();
block_info_list(1).connected_bc_list(1).my_block_id     = 1;
block_info_list(1).connected_bc_list(1).my_face_id      = 3;
block_info_list(1).connected_bc_list(1).nbor_face_id    = 4;
block_info_list(1).connected_bc_list(1).my_bnd_min(:)   = [   1  ,  1  ,  1  ];
block_info_list(1).connected_bc_list(1).my_bnd_max(:)   = [ Nc(1),  1  ,  1  ];
block_info_list(1).connected_bc_list(1).nbor_block_id   = 1;
block_info_list(1).connected_bc_list(1).nbor_bnd_min(:) = [   1  , Nc(2),  1  ];
block_info_list(1).connected_bc_list(1).nbor_bnd_max(:) = [ Nc(1), Nc(2),  1  ];

block_info_list(1).connected_bc_list(2)                 = connected_bc_info_t_old();
block_info_list(1).connected_bc_list(2).my_block_id     = 1;
block_info_list(1).connected_bc_list(2).my_face_id      = 4;
block_info_list(1).connected_bc_list(2).nbor_face_id    = 3;
block_info_list(1).connected_bc_list(2).my_bnd_min(:)   = [   1  , Nc(2),  1  ];
block_info_list(1).connected_bc_list(2).my_bnd_max(:)   = [ Nc(1), Nc(2),  1  ];
block_info_list(1).connected_bc_list(2).nbor_block_id   = 1;
block_info_list(1).connected_bc_list(2).nbor_bnd_min(:) = [   1  ,  1  ,  1  ];
block_info_list(1).connected_bc_list(2).nbor_bnd_max(:) = [ Nc(1),  1  ,  1  ];
...
inputs.epsilon    = 0.1;
inputs.kappa      = -0.0;
inputs.tau        = 0.174532925199428;
inputs.rhoinf     = 1.0;
inputs.pinf       = 1.0;%100000.0;
inputs.vinf       = 1.0;%75.0;
inputs.alpha      = 0.0;

% q_scale = [1.0,75.0,75.0,75.0,100000.0];
q_scale = [1,1,1,1,1];
select = @(M,r) M(r);
qfun1 = @(x)airfoil_prim(x,inputs);
...
rec1 = reconstruct_mod_t_old(); rec1.least_squares.LS_factor = LS_Factor;
...
mean_factor = 10;
boundary_factor = 10;
rec2 = reconstruct_mod_t_old(); rec2.least_squares.LS_factor = LS_Factor;
...
rec3 = reconstruct_mod_t_old(); rec3.least_squares.LS_factor = LS_Factor;
...
mask = sum(rec3.polynomial.exponents,1)<=0;
pfun3_lim2 = @(x)rec3.polynomial.evaluate_limited(x,mask);

mask = sum(rec3.polynomial.exponents,1)<=1;
pfun3_lim3 = @(x)rec3.polynomial.evaluate_limited(x,mask);
...
sub_stencil1 = stencil(:,1:n_stencil);
sub_stencil2 = stencil(:,1:5);
[soln_ex,grid_in_stencil] = eval_soln_in_stencil(n_var,qfun1, q_scale, Nplot(1),Nplot(2),Nplot(3),sub_stencil1,GRID);

[soln_ex1,grid_in_stencil1] = eval_soln_in_stencil(n_var,qfun1, q_scale, Nplot(1),Nplot(2),Nplot(3),sub_stencil2,GRID);
...
[soln_p1,~]               = eval_soln_in_stencil(n_var,pfun3_lim2, q_scale, Nplot(1),Nplot(2),Nplot(3),sub_stencil2,GRID);
[soln_p2,~]               = eval_soln_in_stencil(n_var,pfun3_lim3, q_scale, Nplot(1),Nplot(2),Nplot(3),sub_stencil2,GRID);
[soln_p3,~]               = eval_soln_in_stencil(n_var,pfun3, q_scale, Nplot(1),Nplot(2),Nplot(3),sub_stencil2,GRID);
...
% plot_stencil_cells(plot_var,k_plane,cellfun(@minus,soln_p1,soln_ex1,'Un',0),grid_in_stencil1,'FaceColor','r','EdgeColor','none');
% plot_stencil_cells(plot_var,k_plane,cellfun(@minus,soln_p2,soln_ex1,'Un',0),grid_in_stencil1,'FaceColor','b','EdgeColor','none');
% plot_stencil_cells(plot_var,k_plane,cellfun(@minus,soln_p3,soln_ex1,'Un',0),grid_in_stencil1,'FaceColor','g','EdgeColor','none');
view(-15,35)
...
function q = airfoil_prim(x,inputs)
w = airfoil_velocity(x(1),x(2),inputs.l,inputs.epsilon,inputs.kappa,inputs.tau,inputs.alpha,inputs.vinf);
q = zeros(5,1);
q(1) = inputs.rhoinf;
q(2) =  real(w);
q(3) = -imag(w);
q(5) = inputs.pinf + 0.5*inputs.rhoinf*(inputs.vinf^2 - q(2).^2 - q(3).^2);
end
