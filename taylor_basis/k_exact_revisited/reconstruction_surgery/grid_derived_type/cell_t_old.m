classdef cell_t_old
    ...
    methods
        function this = cell_t_old( block, idx, block_info_list )
            if nargin ~= 3
                return
            else
    ...
    methods (Static)
        ...
        function mask = face_on_a_boundary(idx,bnds_min,bnds_max)
            ...
            for d = 1:3
                for s = 1:2
                    bnds_tmp = bnds;
                    bnds_tmp(d,:) = bnds(d,s);
                    mask(2*(d-1)+s) = cell_t_old.in_bound(3,idx,bnds_tmp(:,1),bnds_tmp(:,2));
                end
            end
        end
        ...
        function [ stencil, n_stencil, stencil_cells ] = build_stencil( block_id, idx, n_stencil, grid, block_info_list, balanced, print_iter )
            ...
            stencil_cells(buffer_size,1) = cell_t_old;
            ...
            while ( n < n_stencil )&&( outer_iter < outer_iter_max )
                ...
                for i = 1:n
                    stencil_cells = cell_t_old.check_neighbors( stencil_cells, i, n );
                end
                % iterate through cells and calculate minimum degree
                [ stencil_idx, face_idx, min_degree ] = cell_t_old.get_grid_distance( grid, center, stencil_cells, n );
                cnt = 0;
                inner_iter = 0;
                ...
                while ( min_degree == current_min_degree )&&(inner_iter < inner_iter_max)
                    ...
                    for i = 1:n+cnt
                        stencil_cells = cell_t_old.check_neighbors( stencil_cells, i, n+cnt );
                    end
