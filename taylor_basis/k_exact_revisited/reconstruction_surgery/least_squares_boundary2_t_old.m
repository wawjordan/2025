classdef least_squares_boundary2_t_old < least_squares_t_old
    properties
       ...
       LHS_p           (1,1) {mustBeInteger} = 0
       A               (:,:) {mustBeNumeric}
       B               (:,:) {mustBeNumeric}
    end
    methods
        function this = least_squares_boundary2_t_old(n_var,dim,max_degree,stencil_cells,grid)
            ...
            this.constraint_eval    = zeros(this.n_constraints,1);
        ...
        function [B,row,constraint_eval] = fill_bc_constraint_rows(this,dim,row,exponents,stencil,grid,B)
            ...
            for n = 1:this.n_boundaries
                ...
                for q = 1:quad.n_quad
                    for ci = 1:n_constraints
                        ...
                        % bc_constraints.constraint(ci).constraint_eval = grid.gblock(b).bounds(n).constraint_eval(ci);
                        ...
                        B(row,:) = bc_constraints.constraint(ci).constraint_row;

                        % store RHS constraint evaluations
                        % constraint_eval(bc_row) = bc_constraints.constraint(ci).constraint_eval;
                        % constraint_eval(row) = bc_constraints.constraint(ci).constraint_eval;
                    end
                end
            end
        ...
        function this = setup( this, n_rhs, exponents, moments, stencil, grid, dim )
            ...
            this.LHS_m = stencil_size * n_vars;
            ...
            this.LHS_p = this.n_constraints;
            this.A = zeros(this.LHS_m,this.LHS_n);
            this.B = zeros(this.LHS_p,this.LHS_n);
            ...
            for nv = 1:n_vars
                [this.B,row] = this.fill_mean_constraint(this.B,row,col_start,exponents,moments);
            ...
            [this.B,~,this.constraint_eval] = fill_bc_constraint_rows(this,dim,row,exponents,stencil,grid,this.B);
            ...
            for nv = 1:n_vars
                ...
                [this.A,row] = this.fill_stencil_rows( this.A, row, col_start, exponents, moments, stencil, grid );
                col_start = col_start + n_terms;
            end
        ...
        function poly = solve(this,soln_in_stencil,poly,~,stencil,grid)
            ...
            for vi = 1:n_vars
                ...
                % fill RHS
                RHS(row) = mean_constraint_factor * fit_vars(vi);
            ...
            for ci = 1:this.n_bc_constraints
                ...
                % fill RHS
                % RHS(row) = constraint_factor * this.constraint_eval(ci);
                RHS(row) = constraint_factor * this.constraint_eval(row);
            ...
            %     end
            % end
            d = RHS(1:this.n_constraints);
            c = RHS(this.n_constraints+1:this.n_constraints+this.LHS_m);
            coeffs = lsqcon_alt_old(this.A, c, this.B, d);
            for vi = 1:n_vars
                for ti = 1:poly.n_terms
                    ...
                    % Compute coefficient
                    poly.coeff(vi,ti) = coeffs(row_iter);
                end
            end
