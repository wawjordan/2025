classdef zero_mean_basis6
    ...
    properties
        ...
        terms      (:,1) monomial_t6             % monomial class
    ...
    methods
        function this = zero_mean_basis6(n_dim,degree,h_ref,quad)
            ...
            this.terms = repmat(monomial_t6(),this.n_terms,1);
        ...
        function x_bar = transform(this,x)
            ...
            x_bar = zero_mean_basis6.shift_and_scale_static( x(1:this.n_dim), this.x_ref, scale_factor );
        end
        ...
        function D = calc_basis_derivative(this,term,deriv,point,scale)
            ...
            L = zero_mean_basis6.get_length_scale(order,scale);
            D = L * derivative_term;
        end

        function deriv = calc_basis_derivatives(this,n1,point,scale)
            % Inputs:
            %  this      - zero_mean_basis6 object
    ...
    methods (Static)
        function L = get_length_scale(order,scale)
            if isscalar(scale)
                L = zero_mean_basis6.get_length_scale_scalar(order,scale);
            else
                L = zero_mean_basis6.get_length_scale_vector(order,scale);
            end
        end
        ...
        function exp = get_exponents_sorted( dimen, total_degree )
            n_terms = zero_mean_basis6.get_n_terms(dimen,total_degree);
            ...
            for n = 0:full_degree
                tmp_exp = zero_mean_basis6.global_to_local(n+1,nSub) - 1;
