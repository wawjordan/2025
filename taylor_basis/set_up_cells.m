function CELLS = set_up_cells(GRID,blk,idx_low,idx_high,degree,n_vars)
IDX = generate_index_array(idx_low,idx_high);

CELLS = arrayfun( @(idx)set_up_cell(GRID,blk,idx,degree,n_vars), IDX );
...
T = taylor_shape_functions(n_dim,degree,nodes,Q);
CELL = cell_soln_info(nbor_idx,T,Q,FQ,FN,n_vars);
end
