function CELLS = set_up_cell_var_recs4(GRID,blk,idx_low,idx_high,degree,funs,n1,use_bcs)
nvars = numel(funs);
...
if (use_bcs)
    CELLS = var_rec_t4.setup_all(n1,CELLS,funs);
else
    CELLS = var_rec_t4.setup_all(n1,CELLS,[]);
end
end
...
[FQ,FN,nbor_face_id,bc_id] = get_face_info(GRID,blk,idx,n_dim);
nodes = get_nodes(GRID,blk,nbor_idx(:,1),n_dim);
h_ref = get_max_cell_extents(nodes,n_dim);
T = zero_mean_basis4(n_dim,degree,h_ref,Q);
CELL = var_rec_t4(nbor_idx,nbor_face_id(1:n_nbor),bc_id,T,Q,FQ,FN,n_vars);
end
