classdef quad_t
    ...
    methods (Static)
        function quad_ref = create_quad_ref_1D(n_quad)
            ...
            [pts_1D,wts_1D] = quad_t.LegendreGaussNodesAndWeights(n_quad);
        ...
        function [x,w] = LegendreGaussNodesAndWeights(N)
            ...
            elseif N == 1
                ...
                for j = 0:floor((N+1)/2)-1
                    ...
                    for k = 1:n_it
                        [LNp1,dLNp1] = quad_t.LegendrePolynomialAndDerivative(N+1,x(j+1));
                        delta = -LNp1/dLNp1;
                        x(j+1) = x(j+1) + delta;
                    ...
                    [~,dLNp1] = quad_t.LegendrePolynomialAndDerivative(N+1,x(j+1));
                    x(N+1-j) = -x(j+1);
                    w(j+1) = 2/( (1-x(j+1)^2)*dLNp1^2);
                    w(N+1-j) = w(j+1);
                end
                if mod(N,2)==0
                    [~,dLNp1] = quad_t.LegendrePolynomialAndDerivative(N+1,0.0);
                    x(N/2+1) = 0;
                    w(N/2+1) = 2/dLNp1^2;
                end
            end
