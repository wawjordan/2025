classdef derived_grid_vars_t
    properties
        ...
        xi_face_quad   (:,:,:) quad_t
        eta_face_quad  (:,:,:) quad_t
        zeta_face_quad (:,:,:) quad_t 
    end
    methods
        function this = allocate_derived_grid( this, i_cells, j_cells, k_cells )
            ...
            this.xi_n   = repmat(face_vec_t(), i_cells+1, j_cells  , k_cells   );
            this.eta_n  = repmat(face_vec_t(), i_cells  , j_cells+1, k_cells   );
            this.zeta_n = repmat(face_vec_t(), i_cells  , j_cells  , k_cells+1 );
            
            this.quad           = repmat(quad_t(), i_cells  , j_cells  , k_cells   );
            this.xi_face_quad   = repmat(quad_t(), i_cells+1, j_cells  , k_cells   );
            this.eta_face_quad  = repmat(quad_t(), i_cells  , j_cells+1, k_cells   );
            this.zeta_face_quad = repmat(quad_t(), i_cells  , j_cells  , k_cells+1 );
        end
    end
end