classdef reference_quad
    ...
    methods
        function this = reference_quad(Nq)
            if nargin > 0
                if numel(Nq)==2
        ...
        function this = create_reference_quad_2D(this,Nq)
            this.e = repmat(sub_quad(),0,1);
            this.f = repmat(sub_quad(),4,1);
            this.v = repmat(sub_quad(),1,1);
            this.Nq(1) = Nq(1);
            this.Nq(2) = Nq(2);
            this.Nq(3) = 1;
        ...
        function this = create_reference_quad_3D(this,Nq)
            this.e = repmat(sub_quad(),12,1);
            this.f = repmat(sub_quad(),6,1);
            this.v = repmat(sub_quad(),1,1);
            this.Nq(1) = Nq(1);
            this.Nq(2) = Nq(2);
            this.Nq(3) = Nq(3);
    ...
    methods (Static)
        function [x,w] = generate_1D_quads(Nmax)
            ...
            w = zeros(Nmax,Nmax);
            for i = 1:Nmax
                [x(1:i,i),w(1:i,i)] = quad_t.LegendreGaussNodesAndWeights(i);
            end
        end
        ...
        function [pts,wts] = edge_quad_2D(N,edge,x1D,w1D)
            ...
            dir2 = mod(dir1,2) + 1;
            Npts = N(dir1);
            end_pt = reference_quad.get_end_pts(mod(edge-1,2)+1,1);
        ...
        function [pts,wts] = edge_quad_3D(N,edge,x1D,w1D)
            ...
            end_pts = reference_quad.get_end_pts(mod(edge-1,4)+1,2);
            pts(dir1,:) = x1D(1:Npts,N(dir1));
            pts(dir2,:) = end_pts(1);
            pts(dir3,:) = end_pts(2);
            wts         = w1D(1:Npts,N(dir1));
        ...
        function [pts,wts] = face_quad_3D(N,face,x1D,w1D)
            ...
            end_pt = reference_quad.get_end_pts(mod(face-1,2)+1,1);
